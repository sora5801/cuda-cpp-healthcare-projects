...
#   stdout (deterministic pattern metrics) is diffed against expected_output.txt;
#   stderr (timing) is shown but not diffed.
# ===========================================================================
$ErrorActionPreference = "Stop"
$Demo        = $PSScriptRoot
$ProjectRoot = Split-Path -Parent $Demo
$Slug        = "spatial-whole-cell-reaction-diffusion-at-molecular-resolution"
$Sln         = Join-Path $ProjectRoot "build\$Slug.sln"
$Exe         = Join-Path $ProjectRoot "build\x64\Release\$Slug.exe"
$Sample      = Join-Path $ProjectRoot "data\sample\grayscott_params.txt"
